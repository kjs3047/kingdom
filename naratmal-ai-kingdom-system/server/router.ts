...
function categorizeRequest(message: string, sensitive?: boolean): RequestCategory {
  ...
  if (/(개발|코드|api|자동화|구현|bot|챗봇|서버)/i.test(message)) return 'engineering';
  if (/(ppt|디자인|ui|ux|화면|슬라이드|브랜드|시각)/i.test(message)) return 'design';
  if (/(기획|전략|로드맵|mvp|구조|설계)/i.test(message)) return 'planning';
  if (/(보고서|문안|카피|소개문|발표|콘텐츠|홍보)/i.test(message)) return 'content';
  return 'operations';
}
...
export function routeRequest(request: UserRequest): RoutingDecision {
  ...
  if (reviewRequired && leadAgent !== 'audit_guard') {
    tasks.push({
      agent: 'audit_guard',
      reason: '출고 전 검수',
      inputSummary: request.message,
  ...
  return { category, leadAgent, supportAgents, reviewRequired, tasks };
}
